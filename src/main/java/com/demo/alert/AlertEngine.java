...
public class AlertEngine {
    ...
    private final LogFormatInspector logFormatInspector;

    public AlertEngine(LogFormatInspector logFormatInspector) {
        this.logFormatInspector = logFormatInspector;
    }
    ...
    public List<Alert> evaluate(List<LogEntry> entries, List<AlertRule> rules, Instant now) {
        ...
        for (AlertRule rule : rules) {
            ...
            List<LogEntry> matched = entries.stream()
                    ...
                    .filter(entry -> logFormatInspector.isRuleLog(entry.message()))
